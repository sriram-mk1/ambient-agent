...
export async function GET(request: Request) {
  try {
    ...
    // Get a fresh token for Docs (Drive scope)
    let accessToken: string | null = null;
    try {
      const { tokenRefreshManager } = await import("@/lib/token-refresh");
      accessToken =
        (await tokenRefreshManager.getFreshToken(user.id, "docs")) ||
    ...
    if (!accessToken) {
      return NextResponse.json(
        { error: "Missing Google access token for Docs/Drive" },
        { status: 400 },
      );
    }

    // Build Drive files list query (Google Docs only)
    const driveQueryParts = [
      "mimeType='application/vnd.google-apps.document'",
      "trashed=false",
    ];
    if (query) {
      // Basic name contains filter; Drive API uses single quotes
    ...
    console.error("/api/apps/docs/list error", error);
